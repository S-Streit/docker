...
def call_extract_features(args):
    ...
    clam_command = "CUDA_VISIBLE_DEVICES=0 python3 /usr/local/src/clam/extract_features_fp.py --data_slide_dir {0} --csv_path {1} --feat_dir {2} --data_h5_dir {3} --batch_size=64 --slide_ext .svs".format(input_folder, csv_path, feat_dir, data_h5_dir)
...
def call_create_heatmaps(args):
    ...
    heatmap_command = "python3 /usr/local/src/clam/create_heatmaps.py --config /usr/local/src/clam/heatmpas/configs/config_docker.yaml"
